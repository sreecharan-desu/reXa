// File: client/src/pages/MyRewards.tsx
...
export const MyRewards = () => {
  ...
  const fetchMyRewards = async () => {
    ...
    try {
      ...
      // we'll compute visible list in the sync effect below
    } catch (err) {
  ...
  // helper - check expiry / availability
  const isNotExpired = (r: Reward) =>
    !r.expiryDate || new Date(r.expiryDate) > new Date();

  const isVisibleByDefault = (r: Reward) =>
    r.status === "available" && isNotExpired(r) && r.isActive !== false;
  ...
  // keep filteredRewards in sync with recoil state and handle loading flag
  useEffect(() => {
    const visible = (rewards || []).filter(isVisibleByDefault);
    setFilteredRewards(visible);
    setLoading(false);
    // setLoading is from recoil setter; include since we call it here
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [rewards]);

  // main filtering - always start from the default-visible set
  useEffect(() => {
    const base = (rewards || []).filter(isVisibleByDefault);

    const filtered = base.filter((reward) => {
      const matchesSearch =
      ...
      const matchesExpiry = statusFilter !== "expired" || isExpiredLocal;
      return (
        matchesSearch &&
        matchesCategory &&
        matchesPrice &&
  ...
  // count visible / total
  const visibleCount = filteredRewards.length;
  const totalOwned = rewards?.length || 0;
  ...
  return (
    <PageLayout title="My Rewards">
      <div className="space-y-6">
        ...
        {/* Info banner - clarifies what's visible */}
        <div className="flex items-center gap-3 bg-blue-50 dark:bg-gray-800 border border-blue-100 dark:border-gray-700 text-sm text-blue-900 dark:text-blue-200 px-4 py-2 rounded-md">
          <FiInfo className="w-5 h-5" />
          <div>
            <div>
              Showing {visibleCount} of {totalOwned} reward
              {totalOwned !== 1 ? "s" : ""} - only rewards that are available,
              not redeemed, and not expired are displayed here.
            </div>
            <div className="text-xs text-gray-600 dark:text-gray-400 mt-1">
              Use filters above to narrow results. If you expect more items,
              check the Redeemed or Expired tabs.
            </div>
          </div>
        </div>
