// File: client/src/pages/RewardDetails.tsx
...
/**
 * RewardDetails page
 * - Uses shared makePlaceholderSvgDataUrl for deterministic placeholder SVGs when image_url is missing or empty.
 * - Placeholder intentionally does NOT include any reward code.
 */
...
type RewardType = {
  ...
  status: "available" | "redeemed" | "exchanged" | string;
  isActive?: boolean;
...
export const RewardDetails = () => {
  ...
  const handleRedeem = async () => {
    ...
    try {
      ...
      // try to extract updated points from a few possible response shapes
      const points =
        response.data?.updatedRedeemingUser?.points ??
        response.data?.updatedRedeemingUser?.points ??
        response.data?.userPoints ??
        response.data?.updatedRedeemingUser?.data?.points;
  ...
  // unified image logic - prefer reward.image_url, otherwise use shared placeholder generator
  const imageSrc =
        ...
        <div className="p-5 flex justify-between items-center">
          <span
            className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium border ${
              statusClasses[reward.status] ||
              "bg-gray-100 text-gray-700 border-gray-200"
            }`}
          >
            {statusIcons[reward.status] || <FiTag className="w-4 h-4" />}
            ...
            {!isOwner && (
              <div className="mt-4">
                <button
                  ...
                  disabled={
                    reward.status !== "available" || reward.isActive === false
                  }
